...
public class Map implements Runnable {
    ...
    try {
        while (true) {
            ...
            // Ensure a minimum delay of 1000 milliseconds
            long remainingDelay = Math.max(1000 - timeDo, 0);
            
            // Sleep for the remaining delay time
            Thread.sleep(remainingDelay);
        }
    } catch (InterruptedException e) {
        Logger.logException(Map.class, e, "Interrupted while updating map " + this.mapName);
        // You might want to add additional handling for the interruption if needed.
    } catch (Exception e) {
        Logger.logException(Map.class, e, "Error updating map " + this.mapName);
    }
}
