...
public class Ruonggapthu {

    private static final int MAX_ITEM_IN_BOX_thu = 500;
    ...
    private static Ruonggapthu i;

    private Ruonggapthu() {

    }

    public static Ruonggapthu gI() {
        if (i == null) {
            i = new Ruonggapthu();
        }
        return i;
    ...
    private void openBallByGem(Player player, byte count) {
        ...
        if (player.inventory.gem < gemNeed) {
            ...
            if (count + player.inventory.itemsBoxCrackBallthu.size() <= MAX_ITEM_IN_BOX_thu) {
