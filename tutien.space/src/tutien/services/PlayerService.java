...
public class PlayerService {
    ...
    private static final int COST_GOLD_HOI_SINH = 5000000;
    private static final int COST_GEM_HOI_SINH = 5000000;
    private static final int COST_GOLD_HOI_SINH_NRSD = 5000000;
        ...
        if (player.isDie()) {
            ...
            if (MapService.gI().isMapBlackBallWar(player.zone.map.mapId)) {
                if (player.inventory.gold >= COST_GOLD_HOI_SINH_NRSD) {
                    ...
                    Service.getInstance().sendThongBao(player, " về tu đi rồi ra solo " + Util.numberToMoney(COST_GOLD_HOI_SINH_NRSD
                            - player.inventory.gold) + " vàng");
                    return;
                }
            }
            if (player.zone.map.mapId == 178 && player.HoiSinhDaiChienBangHoi == 1) {
                canHs = false;
                Service.gI().sendThongBaoOK(player, "Bạn đã hết lượt hồi sinh");
                return;
            }if (player.zone.map.mapId == 178 && player.HoiSinhDaiChienBangHoi != 1) {
                canHs = true;
                player.HoiSinhDaiChienBangHoi -= 1;
                Service.gI().sendThongBao(player, "Bạn còn " + player.HoiSinhDaiChienBangHoi +" lượt hồi sinh");
            }
            else {
                if (player.inventory.gold >= COST_GOLD_HOI_SINH) {
                    ...
                    Service.getInstance().sendThongBao(player, "về hổi sinh đi ra bú tiếp " + Util.numberToMoney(COST_GOLD_HOI_SINH
                            - player.inventory.gold) + " vàng");
                    return;
                }
            }
