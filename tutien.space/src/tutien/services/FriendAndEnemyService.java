    ...
    private void makeFriend(Player player, int playerId) {
        ...
        for (Friend friend : player.friends) {
            if (friend.id == playerId) {
                Service.getInstance().sendThongBao(player, "Đã có trong danh sách đen");
                madeFriend = true;
                break;
            }
        }
        if (!madeFriend) {
            ...
            if (pl != null) {
                ...
                if (player.friends.size() >= 5) {
                    npcSay = "mày có muốn kết bạn với " + pl.name + " với phí là 5 tỷ ngọc ?";
                } else {
    ...
    private void removeFriend(Player player, int playerId) {
        for (int i = 0; i < player.friends.size(); i++) {
            if (player.friends.get(i).id == playerId) {
                Service.getInstance().sendThongBao(player, "Đã band thành công "
                        + player.friends.get(i).name + " khỏi mắt mày");
                Message msg;
                try {
                    msg = new Message(-80);
            ...
            if (pl != null) {
                if (player.isAdmin() || player.nPoint.teleport) {
                    if (!pl.itemTime.isUseAnDanh || player.isAdmin()) {
                        if ((player.isAdmin() || !pl.zone.isFullPlayer()) && !MapService.gI().isMapDoanhTrai(pl.zone.map.mapId) && !MapService.gI().isMapGiaiCuuMiNuong(pl.zone.map.mapId) && !MapService.gI().isMapMaBu(pl.zone.map.mapId)) {
